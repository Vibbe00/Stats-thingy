...
async function backfill() {
    ...
    const { rows } = await db.query(
        ...
        WHERE item0 = 0 AND item1 = 0 AND item2 = 0 AND item3 = 0 AND item4 = 0 AND item5 = 0 AND item6 = 0 OR team_position = '' OR team_position IS NULL`
    );
        ...
        try {
            ...
            for (const p of match.info.participants) {
                await db.query(
                    ...
                    SET item0 = $1, item1 = $2, item2 = $3, item3 = $4, item4 = $5, item5 = $6, item6 = $7, team_position = $8
                    WHERE match_id = $9 AND puuid = $10`,
                    [p.item0, p.item1, p.item2, p.item3, p.item4, p.item5, p.item6, p.teamPosition, matchId, p.puuid]
                );
            }
